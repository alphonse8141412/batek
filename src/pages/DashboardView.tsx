  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="flex gap-2">
        <button className="px-3 py-1.5 text-[11px] font-medium rounded-full border border-foreground text-foreground">
          Consigne: Oignons en Terre
        </button>
        <button className="px-3 py-1.5 text-[11px] font-medium rounded-full border border-border text-muted-foreground">
          IA / Prédiction
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <KpiCard icon={<Droplets size={16} />} value="58" unit="%" label="Prédiction IA" status="Actif" statusType="actif" />
        <KpiCard icon={<AlertTriangle size={16} />} value="Moy" unit="" label="Risque Détériora." status="Optimal" statusType="optimal" />
      ...
      <div className="bg-card rounded-lg p-3">
        ...
        <div className="flex justify-center gap-4 mt-1">
          <span className="flex items-center gap-1 text-[10px] text-primary">● Oignon</span>
          <span className="flex items-center gap-1 text-[10px] text-accent">● Patate</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="alert-critique-card rounded-lg p-3">
          <p className="text-destructive text-xs font-semibold">Alertes Critiques</p>
          <p className="text-[10px] text-muted-foreground">Tour froide n°17 · Température hors limite Zone 3</p>
        </div>
        <div className="alert-vigilance-card rounded-lg p-3">
        ...
        {[
          ...
          <div key={zone.name} className="bg-card rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-full badge-${zone.status === "Actif" ? "actif" : "vigilance"}`}>
                {zone.status}
              </span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-center mb-2">
              <div>
                <p className="text-sm font-bold text-foreground">{zone.temp}</p>
                <p className="text-[9px] text-muted-foreground">Tm</p>
              </div>
              <div>
                <p className="text-sm font-bold text-foreground">{zone.hum}</p>
                <p className="text-[9px] text-muted-foreground">Humidité</p>
              </div>
              <div>
                <p className="text-sm font-bold text-foreground">{zone.thresh}</p>
